...
class VueJsLayoutPublishCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'thomisticus.publish:vuejs';
    ...
    {
        $viewsPath = config('thomisticus.crud_generator.path.views', base_path('resources/views/'));
        $resourcesPath = config('thomisticus.crud_generator.path.resourcesPath', base_path('resources/'));
        $vendorPath = $resourcesPath . 'assets/vendor/';
        $assetsJsPath = config('thomisticus.crud_generator.path.assetsJsPath', base_path('resources/assets/js/'));
        $assetsCssPath = config('thomisticus.crud_generator.path.assetsCssPath', base_path('resources/assets/css/'));
        $templateType = config('thomisticus.crud_generator.templates', 'core-templates');
        $requestPath = config('thomisticus.crud_generator.path.api_request', base_path('app/Http/Requests/'));
        ...
        foreach ($files as $stub => $blade) {
            $sourceFile = base_path('vendor/thomisticus/' . $templateType . '/templates/' . $stub);
            $destinationFile = $viewsPath . $blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($vueLayoutFiles as $stub => $blade) {
            $sourceFile = base_path('vendor/thomisticus/' . $templateType . '/templates/vuejs/' . $stub);
            $destinationFile = $viewsPath . $blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($baseRequestCustomFiles as $stub => $php) {
            $sourceFile = base_path('vendor/thomisticus/crud-generator/templates/' . $stub);
        ...
        $sourceFile = base_path('vendor/thomisticus/crud-generator/templates/vuejs/js/gulpfile.js');
        $destinationFile = base_path() . '/gulpfile.js';
        $this->publishFile($sourceFile, $destinationFile, 'gulpfile.js');

        $sourceFile = base_path('vendor/thomisticus/crud-generator/templates/vuejs/js/package.json');
