...
namespace Thomisticus\Generator\Generators\Vuejs;
...
class RoutesGenerator extends BaseGenerator
{
    ...
    public function __construct(CommandData $commandData)
    {
        ...
        if (!file_exists($this->pathApi)) {
            file_put_contents($this->pathApi, get_template('vuejs.routes.api_routes', 'crud-generator'));
        }

        $this->apiRouteContents = file_get_contents($this->pathApi);

        $routesTemplate = get_template('vuejs.routes.routes', 'crud-generator');
        $apiRoutesTemplate = get_template('vuejs.routes.api_routes_base', 'crud-generator');
